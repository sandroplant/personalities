...
const messagingRoutes = require('./src/routes/messaging'); // Messaging routes
...
const io = new Server(server, {
  cors: {
    origin: process.env.CLIENT_URL || 'http://localhost:3000', // Replace with your client-side URL
    methods: ['GET', 'POST'],
    credentials: true
  }
});
...
app.use('/messaging', messagingRoutes); // Messaging routes integration
app.use('/messaging/ai', aiRoutes); // AI routes integration
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMedia', ({ roomId, mediaData, mediaType, sender }) => {
    // Broadcast the media to other users in the room
