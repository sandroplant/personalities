...
const getSpotifyData = async (accessToken) => {
    try {
        ...
        const [meResponse, topArtistsResponse, topTracksResponse, currentlyPlayingResponse] = await Promise.all([
            spotifyApi.getMe(),
...
router.get('/', ensureAuthenticated, profileRateLimiter, sanitizeInput, async (req, res) => {
    ...
    try {
        ...
        let user = await User.findOne({ spotifyId: sanitizedSpotifyData.spotifyId }, null, { sanitizeFilter: true });
        if (!user) {
            user = new User({
...
router.post('/create', ensureAuthenticated, profileRateLimiter, verifyCsrfToken, sanitizeInput, [
    ...
    try {
        ...
        let profile = await Profile.findOne({ user: sanitizedUser }, null, { sanitizeFilter: true });
        if (profile) {
