...
const app = express();
// Define the port
const PORT = parseInt(process.env.PORT, 10) || 80; // Default to port 80 if PORT not set
// Create HTTP server
...
app.post('/profile', rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 50,
    message: 'Too many profile update requests from this IP, please try again after 15 minutes',
}), verifyCsrfToken, body('userId').isString().trim().notEmpty().withMessage('User ID is required'), body('name').isString().trim().notEmpty().withMessage('Name must be a string'), body('bio').isString().trim().notEmpty().withMessage('Bio must be a string'), async (req, res) => {
    if (process.env.NODE_ENV !== 'production') {
    ...
    try {
        const updatedUser = await User.findByIdAndUpdate(validUserId, {
            name,
            bio,
