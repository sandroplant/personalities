...
interface SpotifyData {
    ...
    currentlyPlaying?: SpotifyApi.CurrentlyPlayingObject | null;
}
...
const getSpotifyData = async (accessToken: string): Promise<SpotifyData> => {
    try {
        ...
        return {
            ...
            currentlyPlaying:
                currentlyPlayingResponse.body as SpotifyApi.CurrentlyPlayingObject | null,
        };
    } catch (error) {
        ...
        try {
            ...
            const sanitizedSpotifyData = {
                ...
                topArtists: spotifyData.topArtists?.map((artist) => ({
                    ...
                    external_urls: Object.fromEntries(
                        Object.entries(artist.external_urls).map(
                            ([key, value]) => [key, value as string]
                        )
                    ) as Record<string, string>,
                    images: artist.images.map((image: { url: string }) => ({
                        url: sanitizeHtml(image.url),
                    })),
                })),
                topTracks: spotifyData.topTracks?.map((track) => ({
                    name: sanitizeHtml(track.name),
                    album: Object.fromEntries(Object.entries(track.album)),
                    external_urls: Object.fromEntries(
                        Object.entries(track.external_urls).map(
                            ([key, value]) => [
                                key,
                    ...
                    ? {
                          name: sanitizeHtml(
                              spotifyData.currentlyPlaying.item &&
                                  'name' in spotifyData.currentlyPlaying.item
                                  ? spotifyData.currentlyPlaying.item.name
                                  : ''
                          ),
                          uri: sanitizeHtml(
                              spotifyData.currentlyPlaying.item &&
                                  'uri' in spotifyData.currentlyPlaying.item
                                  ? spotifyData.currentlyPlaying.item.uri
                                  : ''
                          ),
                          album: sanitizeHtml(
                              spotifyData.currentlyPlaying.item &&
                                  'album' in spotifyData.currentlyPlaying.item
                                  ? spotifyData.currentlyPlaying.item.album.name
                                  : ''
                          ),
                      }
                    : null,
            ...
            if (!user) {
                user = new User({
                    ...
                    topArtists: sanitizedSpotifyData.topArtists,
                    topTracks: sanitizedSpotifyData.topTracks,
                    currentlyPlaying: sanitizedSpotifyData.currentlyPlaying,
                });
            } else {
                user.displayName =
                    sanitizedSpotifyData.displayName || user.displayName;
                user.email = sanitizedSpotifyData.email || user.email;
                user.images = sanitizedSpotifyData.images || user.images;
                user.topArtists =
                    sanitizedSpotifyData.topArtists || user.topArtists;
                user.topTracks =
                    sanitizedSpotifyData.topTracks || user.topTracks;
                user.currentlyPlaying = sanitizedSpotifyData.currentlyPlaying
                    ? sanitizedSpotifyData.currentlyPlaying
...
router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            let profile = await Profile.findOne({ user: sanitizedUser }, null, {
                sanitizeFilter: true,
            });
