...
app.get('/test-db', async (_req: Request, res: Response) => {
    try {
        // Replace 'test_spotify_id' with a valid spotifyId or adjust as necessary
...
const authCallbackHandler: RequestHandler = async (req, res) => {
    ...
    try {
        ...
        let user: IUser | null = await User.findOne({ spotifyId }).exec();
        if (!user) {
            user = new User({
                spotifyId,
                displayName,
                email,
                username: '', // Provide default values for missing properties
                password: '',
                images: [],
                topArtists: [],
                topTracks: [],
                ...
                musicPreferences: [],
                favoriteMovies: [],
                friendEvaluations: [], // Add default values for missing properties
            ...
            logger.info(
                `✅ Existing user found: ${displayName} (${spotifyId})`
            );
        }
        ...
        req.session.userId = (user._id as mongoose.Types.ObjectId).toString(); // Assuming user._id is an ObjectId
        // No longer need to cast req.session as any
...
app.get('/profile', async (req: Request, res: Response) => {
    if (!req.session.access_token) {
        logger.warn(
            '❌ Access token missing in session. Redirecting to /login'
        );
    ...
    try {
        ...
        logger.info(
            `✅ Profile data fetched for user: ${profileData.display_name}`
        );
...
app.post(
    ...
    rateLimit({
        ...
        message:
            'Too many profile update requests from this IP, please try again after 15 minutes',
    }),
    verifyCsrfToken,
    body('userId')
        .isString()
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            const updatedUser = await User.findByIdAndUpdate(
                ...
                { new: true, runValidators: true, sanitizeFilter: true } // Note: sanitizeFilter removed if not needed
            );
            ...
            logger.info(
                `✅ Profile updated successfully for user: ${updatedUser.displayName}`
            );
            res.status(200).json({
