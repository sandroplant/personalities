...
} from '../utils/spotifyAuthUtils'; // Adjusted import path, no .js
...
router.get('/profile', ensureAuthenticated, async (req: Request, res: Response) => {
    ...
    if (!accessToken) {
        return res.status(401).send('Access token missing. Please log in again.');
    }
    ...
    try {
        ...
        const [topArtistsResponse, topTracksResponse, currentTrackResponse] = await Promise.all([
            ...
            axios.get('https://api.spotify.com/v1/me/top/tracks?limit=20', { headers }),
        ...
        const profileData = {
            top_artists: Array.isArray((topArtistsData as { items: any[] }).items)
                ? (topArtistsData as { items: any[] }).items.map((artist: any): SpotifyArtist => ({
                      name: artist.name,
                      uri: artist.uri,
            ...
            top_tracks: Array.isArray((topTracksData as { items: any[] }).items)
                ? (topTracksData as { items: any[] }).items.map((track: any): SpotifyTrack => ({
                      name: track.name,
                      uri: track.uri,
                      album: track.album && track.album.name ? track.album.name : '',
                  }))
                : [],
            currently_playing: currentTrackData && typeof currentTrackData === 'object' && 'item' in currentTrackData
                ? {
                      ...
                      album: (currentTrackData as any).item.album && (currentTrackData as any).item.album.name
