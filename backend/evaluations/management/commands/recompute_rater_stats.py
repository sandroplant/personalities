...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--subject-field",
            type=str,
            help="Evaluation FK field name for subject user",
        )
        parser.add_argument(
            "--rater-field",
            type=str,
            help="Evaluation FK field name for rater user",
        )
        parser.add_argument(
            "--criterion-field",
            type=str,
            help="Evaluation FK field name for criterion",
        )
        parser.add_argument(
            "--score-field",
            type=str,
            help="Evaluation numeric score field name",
        )
        parser.add_argument(
            "--list-fields",
            action="store_true",
            help="List detected Evaluation fields and exit",
        )

    def handle(self, *args, **options):
        Evaluation = apps.get_model("evaluations", "Evaluation")

        # RaterStats is optional; command should still run if it doesn't exist
        try:
            RaterStats = apps.get_model("evaluations", "RaterStats")
        except Exception:
            RaterStats = None  # type: ignore[assignment]

        # EvaluationMeta is optional filter
        try:
        ...
        user_fk_fields: List[str] = []
        other_fk_fields: List[tuple[str, str]] = []
        ...
        for f in all_fields:
            ...
            elif hasattr(f, "get_internal_type") and f.get_internal_type() in {
                "IntegerField",
                "FloatField",
                "DecimalField",
                "PositiveIntegerField",
        ...
        if not all([subject_field, rater_field, criterion_field, score_field]):
            self.stderr.write(
                "Could not infer Evaluation model fields. "
                "Run with --list-fields and pass "
                "--subject-field/--rater-field/--criterion-field/--score-field."
            )
            return

        # Base queryset; optionally filter by EvaluationMeta status
        ...
        for ev in qs.values(
            ...
            uid_val = ev.get(f"{rater_field}_id")
            if uid_val is None:
                continue
            uid = int(uid_val)
        ...
        # Write aggregates if RaterStats exists; otherwise, just report
        updated = 0
        if RaterStats is not None:
