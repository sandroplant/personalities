...
def callback(request):
    ...
    if not code or not isinstance(code, str):
        return JsonResponse({'error': 'Code is required and must be a string'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return JsonResponse({'message': 'Successfully authenticated with Spotify'}, status=status.HTTP_200_OK)

    except Exception as e:
        return JsonResponse({'error': f'Error during token exchange: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
