...
class MessagingView(View):
    ...
    @api_view(['POST'])
    async def send_message(self, request):  # Ensure the method is async
        ...
        ai_response = await get_ai_response(content) if include_ai else None

        try:
            new_message = Message(sender=sender_id, recipient=recipient_id, content=content, ai_response=ai_response)
            new_message.save()
            return JsonResponse({'message': 'Message sent successfully!', 'aiResponse': ai_response}, status=status.HTTP_200_OK)
