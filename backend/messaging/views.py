...
from .models import (
    Message,
)  # Adjust the import based on your Message model location
...
class MessagingView(View):
    ...
    def get_conversation(self, request, userId1, userId2):
        try:
            ...
            # Serialize the messages (consider creating a serializer
            # for this)
            messages_data = [
                message.serialize() for message in messages
            ]  # Assuming you have a serialize method
            return JsonResponse(
                messages_data, safe=False, status=status.HTTP_200_OK
            )
        except Exception as e:
            return JsonResponse(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
